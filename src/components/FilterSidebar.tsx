...
export function FilterSidebar({ filters, onChange, facetCounts }: FilterSidebarProps) {
  ...
  useEffect(() => {
    if (activeHidden) setProjectsExpanded(true)
  }, [activeHidden, filters.project])

  const visibleProjects =
    projectsExpanded || !hasOverflow ? projects : projects.slice(0, VISIBLE_PROJECT_COUNT)
    ...
    <aside className="w-44 shrink-0 overflow-y-auto border-r border-zinc-800 bg-zinc-950/50 p-3">
      ...
      <FilterSection title="Project">
        ...
        {hasOverflow && (
          ...
          >
            <span>{projectsExpanded ? 'Show less' : `Show all (${projects.length})`}</span>
            <span className="ml-2 shrink-0 text-zinc-600" aria-hidden="true">
              {projectsExpanded ? '▴' : `+${hiddenCount}`}
            </span>
          </button>
        )}
      </FilterSection>
